...
namespace WaveFunctionCollapse
{
    public class Fingerprint
    {
        ...
        public Fingerprint(Mesh mesh, Quaternion orientation, bool top = false)
        {
            ...
            Rotated = Enumerable.Range(0, 4).Select(i => Fingerprint.create(mesh, orientation, i, false, top)).ToArray();
            Base = Rotated[0];
            Flipped = Fingerprint.create(mesh, orientation, 0, flipX: true);

            Symmetric = Fingerprint.Compare(Base, Flipped);
            Invariant = Fingerprint.Compare(Rotated[0], Rotated[1]);
        }
        ...
        private static Vector4[] create(Mesh mesh, Quaternion rotation, int index, bool flipX = false, bool top = false)
        {
            ...
            for (int t = 0; t < triangles.Length - 2; t += 3)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (v1.z > Generator._SlotSize / 2f * 0.99f && v2.z > Generator._SlotSize / 2f * 0.99f)
                    {
                        if (flipX)
                        {
                            v1.x *= -1f;
